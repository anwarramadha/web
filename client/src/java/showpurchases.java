 ...
 */
@WebServlet(urlPatterns = {"/showpurchases"})
public class showpurchases extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            try {
                java.util.List<Purchases> result = getPurchases(uname);
                result.forEach(new Consumer<Purchases>() {
                    ...
                    public void accept(Purchases answer) {
                        ...
                        out.print("<br><br><form role=\"form\" action=\"\" enctype=\"multipart/form-data\" class=\"market\">"
                                ...
                                + "<span class=\"product-buyer\">bought from <b>" + answer.getUnameSeller() + "</b></span>"
    ...
    private java.util.List<marketplace.Purchases> getPurchases(java.lang.String username) throws ClassNotFoundException_Exception {
        // Note that the injected javax.xml.ws.Service reference as well as port objects are not thread safe.
        // If the calling of port operations may lead to race condition some synchronization is required.
        marketplace.PurchasesWS port = service.getPurchasesWSPort();
        return port.getPurchases(username);
    }
